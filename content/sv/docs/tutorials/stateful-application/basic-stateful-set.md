...
<!-- overview -->
Denna handledning ger en introduktion till hantering av applikationer med
{{< glossary_tooltip text="StatefulSets" term_id="statefulset" >}}.
Det visar hur man skapar, raderar, skala och uppdaterar Pods med StatefulSets.


## {{% heading "Förberedelse" %}}

Innan du börjar denna handledning bör du bekanta dig med
Efter Kuberneteskoncept:
...
{{< note >}}
Denna handledning antar att ditt kluster är konfigurerat till dynamiskt tillhandahållande
Persistentvolumes.Om ditt kluster inte är konfigurerat för att göra det, du
måste manuellt tillhandahålla två 1 GIB -volymer innan du startar detta
handledning.
{{< /note >}}

## {{% heading "Mål" %}}

StatefulSets är avsedda att användas med statliga applikationer och distribueras
system.Administrationen av statliga ansökningar och
Distribuerade system på Kubernetes är ett brett, komplext ämne.För att
demonstrera de grundläggande funktionerna i en statlighet och inte för att konflikt
Ämne med det senare kommer du att distribuera en enkel webbapplikation med en StatefulSet.

Efter denna guide kommer du att känna till följande.

* Hur man skapar en StatefulSet
* Hur ett StatefulSet hanterar sina Pods
* Hur man raderar ett StatefulSet
* Hur man storleksjusterar ett StatefulSet
* Hur man uppdaterar ett StatefulSets poddar 

<!-- lessoncontent -->

## Skapa ett StatefulSet

Börja med att skapa en StatefulSet med exemplet nedan. Det är liknande detta
[StatefulSets](/docs/concepts/workloads/controllers/statefulset/) koncept.
Den skapar en [headless Service](/docs/concepts/services-networking/service/#headless-services),
`nginx`, att publicera IP -adresserna för Pods i StatefulSet, `web`.

{{< codenew file="application/web/web.yaml" >}}

Ladda ner exemplet ovan och spara det i en fil som heter `web.yaml`

Du måste använda två terminalfönster.I den första terminalen, använd
[`kubectl get`](/docs/reference/generated/kubectl/kubectl-commands/#get) att titta på skapelsen
av StatefulSettets Poddar.

```shell
kubectl get pods -w -l app=nginx
```

I den andra terminalen, använd
[`kubectl apply`](/docs/reference/generated/kubectl/kubectl-commands/#apply) för att skapa ett
headless Service och StatefulSet som defineras i `web.yaml`.
...
Kommandot ovan skapar två poddar, var och en kör en
[NGINX](https://www.nginx.com) webbserver. Få ut `nginx` Service...
```shell
...
```
...och sedan `web` StatefulSet, för att verifiera att båda lyckades skapas:
```shell
...
För ett StatefulSet med _n_ replicas, när poddar deployas så skapas de sekventiellt och sorteras från _{0..n-1}_. 
Granska terminalsvaret från `kubectl get` kommandot i den första terminalen. 
Förr eller senare så kommer svaret att efterliknande exemplet nedan.
...
Notera att `web-1` podden inte körs förens `web-0` podden gör är
_Running_ (se [Pod Phase](/docs/concepts/workloads/pods/pod-lifecycle/#pod-phase))
och _Ready_ (se `type` in [Pod Conditions](/docs/concepts/workloads/pods/pod-lifecycle/#pod-conditions)).

## Pods i ett StatefulSet

Poddar i ett StatefulSet har ett unikt ordningsindex och en stabil nätverksidentitet.

### Examining the Pod's Ordinal Index

Få ett StatefulSet's Pods:
...
Som nämnts i [StatefulSets](/docs/concepts/workloads/controllers/statefulset/)
så har poddar i ett StatefulSet en klistrad, unik identitet. 
Denna identitet är baserad på ett unikt ordningsindex som tillskrivs varje pod i ett
StatefulSet {{< glossary_tooltip term_id="controller" text="controller">}}.  
Poddarnas namn tar formen `<statefulset name>-<ordinal index>`.
Då `web` StatefulSet har två replikor så skapas två pods, `web-0` and `web-1`.

### Användning av Stable Network Identities

Varje pod har ett stabilt hostname baseras på ditt ordningsindex. Kör
[`kubectl exec`](/docs/reference/generated/kubectl/kubectl-commands/#exec) för att starta
`hostname` kommandot för varje pod:
...
Använd [`kubectl run`](/docs/reference/generated/kubectl/kubectl-commands/#run) to execute
